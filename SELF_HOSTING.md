...
- **Deterministic local/private testing:** set `APP_ENV=development` and `MULTICA_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`, unless you also set the fork-only `MULTICA_ALLOW_FIXED_CODE_IN_PRODUCTION=true` for an internal instance that outsiders cannot reach.
