...
- Tasks already queued wait for the runtime to recover. They fail only once it has stopped heartbeating for longer than the reconnect grace, so a runtime that is merely busy keeps its backlog.
