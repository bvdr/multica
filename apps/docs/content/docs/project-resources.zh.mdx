...
- **后续对话接着上一轮继续。** 同一个 issue 的下一条评论会重新 check out 这个分支，智能体站在自己刚交付的成果上继续做，而不是从 `HEAD` 重新开始、把上一轮的改动落在另一个分支上。这期间你在自己目录里改的东西会被重放到分支上；如果和智能体已经写好的内容冲突，这一轮就在分支当前状态上继续跑，运行时日志里会写明哪些文件没能带过来。把分支 merge 掉之后，下一轮又会从你的 `HEAD` 重新开始。同一个 issue 上有两次执行重叠时，后一个会从前一个的成果分叉到 `agent/<agent>/<issue>-<task>`，因为 git 规定一个分支只能有一棵 worktree。
