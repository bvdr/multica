...
description: ワークスペースごとにセルフホストの Forgejo、Gitea、GitLab インスタンスを接続し、issue 番号を参照する Pull Request / Merge Request を対応する issue へ自動的に紐づけ、マージ時に「完了」へ移動して CI ステータスを表示します。
---
...
Multica はワークスペースごとにセルフホストの Git ホスティング、**Forgejo**、**Gitea**、**GitLab** のいずれかへ接続します。接続後、ブランチ名、タイトル、本文に issue 番号（例: `MUL-123`）を含む Pull Request（GitLab では Merge Request）は、対応する[issue](/issues)へ**自動的に紐づけられ**、issue サイドバーの **Pull requests** に表示されます。終了キーワードを含む PR がマージされると、issue は「完了」へ移動します。head commit の CI はカード上のチェックバーとして表示されます。
...
- **Issue の紐づけ** — タイトル、本文、ブランチの番号で PR を issue へ紐づけます。マージ済み PR に終了キーワード（`Closes`/`Fixes`/`Resolves MUL-123`）があり、open の関連 PR が残っていなければ、issue を「完了」へ移動します。
