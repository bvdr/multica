...
| `MULTICA_ALLOW_FIXED_CODE_IN_PRODUCTION` | empty | Fork-only. `true` honors the fixed code under `APP_ENV=production`; only for internal instances behind an access gate |
