...
| `queued_expired` | The runtime that owned the queued task stopped heartbeating for longer than the reconnect grace | Confirm the runtime is online, then retry |
...
| `queued` | Waiting for a runtime to claim it | Waits as long as its runtime keeps heartbeating; fails only after that runtime has been silent past the reconnect grace, and is not retried automatically |
