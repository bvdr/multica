...
- **Only branches Multica owns are continued.** Continuing a branch is decided by an ownership record, not by its name. A branch you created yourself that happens to be called `agent/<agent>/<issue>` is never checked out, appended to, or read as a previous run — the task takes `agent/<agent>/<issue>-<id>` instead. The same rule keeps two workspaces that mint the same issue identifier off one branch.
