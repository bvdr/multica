...
- **Everyday noun** — the word a user would say out loud for it. **Translate fully**, whether or not it is a database entity: `workspace` → 工作区, `project` → 项目.
- **Protected term** — a core Multica entity that keeps its English name in **every** locale (`issue`), a concept no local word carries (`skill`), or a schema-level identifier a user may have to type or match (`todo`, `in_progress`, `task_id`).

The Chinese pages under `apps/docs/content/docs/*.zh.mdx` are the de facto voice standard for everything else on this page.

### `issue` is a protected term — never translate it

`issue` is the name of a core Multica object, not a description of one. It keeps its English form in every locale:

| Entity | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| the filed unit of work (`issue`) | Issue | **issue** | **issue** | **issue** |
| one agent execution run (`task`) | Task | `task` | **作業** | **작업** |

The rule is one word for one concept, across every surface a user touches. The CLI is `multica issue`, the slash command is `/issue`, the API field is `issue_status`, and an issue key reads `MUL-123` — a translated UI would force every non-English user to carry a mental lookup table between the board and the terminal. Translating it also collided with `task`: `issue` → 任务 and an agent run both landed on the same Chinese word, and an issue can carry many runs.

Casing follows ordinary English orthography, exactly as the `*.zh.mdx` pages already write it:

- Inside a translated phrase, lowercase — `新建 issue`, `子 issue`, `{{count}} 个 issue`, `하위 issue`, `サブ issue`. This matches how `task` and `skill` already render.
- At the start of a label or sentence, capitalize — `Issue 标题`, `Issue 状态`, `Issue を作成`.
- When the string is only the entity name, keep the English source verbatim, including its plural — nav `Issues`, column header `Issue`.
...
- **`issue` in the "problem" sense** (runtime health) is a plain noun, not the entity: `{{count}} issues` on a machine card is `{{count}} 个异常` / `問題 {{count}} 件` / `문제 {{count}}개`, never the entity term.

Everyday nouns still get translated. They follow the "does a settled local word exist" test:
...
- "Create new issue" → "新建 issue"（`issue` is English, so it takes a space)
