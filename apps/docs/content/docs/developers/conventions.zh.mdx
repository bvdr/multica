...
- **日常词** —— 用户会直接说出口的词。**完整翻译成中文**，是不是数据库实体都一样：`workspace` → 工作区、`project` → 项目。
- **受保护术语** —— 全语言保留英文原文的 Multica 核心对象（`issue`）、中文里没有对应词的概念（`skill`），或用户可能要照着敲、照着比对的 schema 标识符（`todo`、`in_progress`、`task_id`）。

其余规则以 `apps/docs/content/docs/*.zh.mdx` 为准 —— docs 是已经实战 20+ 篇的 CN voice 标准。

### `issue` 是受保护术语，任何语言都不翻译

`issue` 是 Multica 核心对象的名字，不是对它的描述。在每一种语言里都保留英文原文：

| 实体 | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| 提交出来的一件事（`issue`） | Issue | **issue** | **issue** | **issue** |
| 智能体的一次执行（`task`） | Task | `task` | **作業** | **작업** |

规则是**一个概念只有一个名字**，用户碰得到的每个界面都一样。CLI 是 `multica issue`，斜杠命令是 `/issue`，API 字段是 `issue_status`，编号写作 `MUL-123` —— 一旦 UI 层单独翻译，非英文用户就得永远背一张「看板 ↔ 终端」的对照表。而且翻译过来还会和 `task` 撞车：`issue` → 任务，智能体的一次执行也落到同一个中文词上，可一个 issue 下面能跑很多次执行。

大小写按英文的常规写法，`*.zh.mdx` 里本来就是这么写的：

- 夹在中文短语里用小写 —— `新建 issue`、`子 issue`、`{{count}} 个 issue`、`하위 issue`、`サブ issue`。和 `task`、`skill` 的写法一致。
- 位于标签或句子开头时首字母大写 —— `Issue 标题`、`Issue 状态`、`Issue を作成`。
- 整条文案就是这个实体名时，照抄英文原文，包括复数 —— 导航 `Issues`、列头 `Issue`。
...
- "Create new issue" → "新建 issue"（`issue` 是英文，前后加空格）
