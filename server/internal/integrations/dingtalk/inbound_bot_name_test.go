...
const verifiedTestBotName = "ContextPRO Bot - Local"
...
func TestInboundFromCallbackWithBotName_PlainTextRemovesOnlyVerifiedBot(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			content:     "@ContextPRO Bot - Local /new inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "bot after chat",
			content:     "/new @ContextPRO Bot - Local inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "bot after body",
			content:     "/new inspect this @ContextPRO Bot - Local",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "repeated bot mentions",
			content:     "@ContextPRO Bot - Local @ContextPRO Bot - Local /new inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
		{
			name:        "other mention before bot",
			content:     "@Alice @ContextPRO Bot - Local /new inspect this",
			want:        "@Alice /new inspect this",
			wantCommand: false,
		},
		{
			name:        "other mention after command",
			content:     "@ContextPRO Bot - Local /new inspect this with @Alice",
			want:        "/new inspect this with @Alice",
			wantCommand: true,
		},
		{
			name:        "different bot preserved",
			content:     "@ContextPRO Bot - DEV @ContextPRO Bot - Local /new inspect this",
			want:        "@ContextPRO Bot - DEV /new inspect this",
			wantCommand: false,
		},
		{
			name:        "longer name preserved",
			content:     "@ContextPRO Bot - Locality /new inspect this",
			want:        "@ContextPRO Bot - Locality /new inspect this",
			wantCommand: false,
		},
		{
			name:        "punctuation-extended name preserved",
			content:     "@ContextPRO Bot - Local-DEV /new inspect this",
			want:        "@ContextPRO Bot - Local-DEV /new inspect this",
			wantCommand: false,
		},
		{
			name:        "embedded literal preserved",
			content:     "quote@ContextPRO Bot - Local /new inspect this",
			want:        "quote@ContextPRO Bot - Local /new inspect this",
			wantCommand: false,
		},
		{
			name:        "mid sentence command stays prose",
			content:     "@ContextPRO Bot - Local please /new later",
			want:        "please /new later",
			wantCommand: false,
		},
		{
			name:        "blank line before command",
			content:     "@ContextPRO Bot - Local\n\n/new inspect this",
			want:        "/new inspect this",
			wantCommand: true,
		},
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			content: `{"richText":[
				...
				{"text":"@ContextPRO Bot - Local /new inspect this"}
			]}`,
			wantText:        "[Image]\ninspect this",
		...
		},
		{
			...
			content: `{"richText":[
				{"text":"@ContextPRO Bot - Local /new ask @Alice"},
				{"type":"picture","downloadCode":"dl-1"}
			]}`,
			wantText:        "ask @Alice\n[Image]",
		...
		},
		{
			...
			content: `{"richText":[
				{"text":"@Alice @ContextPRO Bot - Local /new inspect this"},
				{"type":"picture","downloadCode":"dl-1"}
			]}`,
...
func TestInboundFromCallbackWithBotName_FailClosedWithoutVerifiedName(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			text: "@ContextPRO Bot - Local /new inspect this",
			want: "@ContextPRO Bot - Local /new inspect this",
		},
		{
		...
		},
		{
			...
			text:    "/new inspect this @ContextPRO Bot - Local",
			want:    "/new inspect this @ContextPRO Bot - Local",
			command: true,
		},
		{
			name:    "rich text preserves unknown bot",
			msgtype: "richText",
			content: `{"richText":[{"text":"@ContextPRO Bot - Local /new inspect this"}]}`,
			want:    "@ContextPRO Bot - Local /new inspect this",
		},
	}
...
func TestInboundFromCallbackWithBotName_DoesNotStripPrivateOrUnaddressedText(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cb.Text.Content = "@ContextPRO Bot - Local /new inspect this"
			msg, ok := inboundFromCallbackWithBotName(cb, "appkey-A", verifiedTestBotName)
...
func TestInboundFromCallbackWithBotName_AllSharedCommandsKeepFirstTextLineContract(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "new",
			content: "@ContextPRO Bot - Local /clear inspect this",
			parse: func(body string) bool {
		...
		{
			name:    "issue",
			content: "@ContextPRO Bot - Local /issue login failed",
			parse: func(body string) bool {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			cb.Text.Content = "@ContextPRO Bot - Local please " + strings.TrimPrefix(tc.content, "@ContextPRO Bot - Local ")
			msg, ok = inboundFromCallbackWithBotName(cb, "appkey-A", verifiedTestBotName)
			if !ok || tc.parse(msg.CommandText) {
