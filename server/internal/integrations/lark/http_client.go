...
const (
	...
	// latency from a self-hosted ContextPRO deployment to feishu.cn.
...
// keyed by app_id so a single ContextPRO server reuses Lark's
...
func bindingPromptTemplate(bindURL string) (string, error) {
	doc := map[string]any{
		...
		"header": map[string]any{
			"template": "blue",
			"title":    map[string]any{"tag": "plain_text", "content": "ContextPRO"},
		},
		"elements": []any{
			map[string]any{
				"tag": "div",
				"text": map[string]any{
					"tag":     "lark_md",
					"content": "你还没有绑定 ContextPRO 账户。点击下方按钮完成绑定后即可使用此 Agent。",
				},
			},
			map[string]any{
				"tag": "action",
