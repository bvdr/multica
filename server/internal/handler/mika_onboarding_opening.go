...
Pick one below, or just tell me what you want to get done right now.`,
	...
	"zh": `你好，欢迎来到 %[1]s。Multica 是一个人和 AI 智能体通过 issue 一起把事情做完的工作区。

我是 %[2]s，这里的 Chief of Staff。我负责把事情理清楚、找到合适的智能体接手，也是你随时可以开口的第一站。

接下来是这样：你说一个目标，我把它变成一个 issue，交给合适的智能体开始跑，你能看着它推进。

从下面选一个开始，或者直接告诉我你现在想做成什么。`,

	"ja": `こんにちは。%[1]s へようこそ。Multica は、人と AI エージェントが issue を通じて実際の仕事を進めるワークスペースです。

私は %[2]s、ここの Chief of Staff です。やることを整理し、適したエージェントに引き継ぎ、いつでも最初に声をかけてもらえる存在でいます。

進め方はこうです。目標をひとこと教えてください。私がそれを issue にして、適したエージェントで動かします。進み方はそのまま見られます。

下から一つ選ぶか、いま進めたいことをそのまま教えてください。`,

	"ko": `안녕하세요, %[1]s에 오신 걸 환영합니다. Multica는 사람과 AI 에이전트가 issue를 통해 실제 일을 함께 진행하는 워크스페이스입니다.

저는 이곳의 Chief of Staff, %[2]s입니다. 할 일을 정리하고, 알맞은 에이전트를 붙이고, 언제든 먼저 말을 걸 수 있는 시작점이 되어 드립니다.

시작은 이렇습니다. 목표를 한 줄로 알려주시면 제가 issue로 만들어 알맞은 에이전트로 실행합니다. 진행 상황은 그대로 보실 수 있어요.
