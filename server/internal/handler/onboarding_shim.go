...
const (
	onboardingAssistantName       = "ContextPRO Helper"
	onboardingIssueTitle          = "Start here: learn ContextPRO with ContextPRO Helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers ContextPRO questions and runs CLI operations."
...
// ContextPRO Helper agent created by this shim. Pre-v3 desktop submits a
...
const onboardingAssistantInstructions = `You are ContextPRO Helper, the built-in AI assistant for this ContextPRO workspace. Your role is to help any member use ContextPRO better — answer questions, give advice, and execute workspace operations on their behalf.

## What ContextPRO is

ContextPRO is an open-source, AI-native team workspace (source: https://github.com/multica-ai/multica). The core idea: AI agents are treated as real teammates — they get assigned issues on a kanban-style board, comment in threads, change status, and run code, exactly like human members. You can also chat directly with agents (chat), group them into squads, and run scheduled or triggered automation (autopilot).
...
const onboardingIssueDescription = `Welcome to ContextPRO.

This is your guided first run. ContextPRO Helper is assigned to this issue and will help you try the core workflow:

1. Read ContextPRO Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention ContextPRO Helper when you want it to continue.
...
//
// Creates or reuses one "ContextPRO Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"   which codex",
		"   codex --version",
		"5. Restart the ContextPRO daemon:",
		...
		"",
		"When the runtime is connected, you can create ContextPRO Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"",
		"运行时连上后，你就可以创建 ContextPRO Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
