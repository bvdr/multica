...
// shellIdentifier is what /bin/sh accepts as a variable name.
...
//
// env is the per-task environment the headless launch hands to the agent
// process (task token, daemon port, private multica config root, PATH, …). An
// interactive session only inherits the daemon's own environment, and without
// these the multica CLI refuses to authenticate inside a task folder (the first
// live tmux task stalled on `multica issue get`, 2026-09-03). Exported at the
// top of the script, sorted for stable output; keys that are not valid shell
// identifiers are dropped rather than quoted into something surprising.
func renderTmuxRunScript(folder, claudePath string, args []string, promptPath, exitCodePath string, env map[string]string) string {
...
func (d *Daemon) runTmuxTask(ctx context.Context, task Task, env *execenv.Environment, assignment *localDirectoryAssignment, claudePath string, opts agent.ExecOptions, agentEnv map[string]string, prompt string, taskLog *slog.Logger) (TaskResult, error) {
