...
const (
	...
	DefaultGCCompletedTaskTTLCloud        = 14 * 24 * time.Hour // 14 days — ContextPRO Cloud bounds completed issue-task env retention by default; see defaultGCCompletedTaskTTL
...
type Config struct {
	...
	GCCompletedTaskTTL             time.Duration         // fully clean inactive issue-task envs completed at least this long ago, regardless of parent issue status (default: 14d on ContextPRO Cloud, 0/disabled elsewhere; local_directory envs are never fully removed)
	...
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on ContextPRO Cloud, false on self-host)
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	//
	// Default is opt-in on ContextPRO Cloud (api.multica.ai) and opt-out for
...
// officialCloudHost is the hostname of ContextPRO's hosted cloud. It's the only
...
// at ContextPRO's hosted cloud. Used to pick defaults that are safe on
...
// with it. On ContextPRO Cloud that trade is ours to make — we operate the nodes,
