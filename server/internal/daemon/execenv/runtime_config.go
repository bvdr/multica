...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the ContextPRO-managed brief
...
// writeRuntimeConfigFile writes the ContextPRO runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the ContextPRO marker
...
// CleanupRuntimeConfig excises the ContextPRO marker block from the runtime
...
// of ContextPRO runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a ContextPRO task would pick up
...
// agent about the ContextPRO runtime environment and available CLI tools.
//
