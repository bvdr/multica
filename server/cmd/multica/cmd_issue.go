...
func init() {
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-ContextPRO-Next-Before response header, printed on stderr as \"Next thread cursor\" / \"Next reply cursor\"; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-ContextPRO-Next-Before-Id response header; must be paired with --before.")
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-ContextPRO-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-ContextPRO-Next-Before-Id"); nbid != "" {
